<template>
  <div class="catalog lg:w-4/5 m-auto w-full">
    ...
    >
      <div class="flex justify-center">Нет избранного</div>
    ...
    >
      <div class="flex flex-col items-center gap-3">
        Авторизируйтесь для добавления товара в избраное
        <UButton
          label="Авторизация"
          size="sm"
          color="primary"
...
const cards = computed(() => {
  pageTotal.value = favoritesStore.favoritesItems.length;
  return favoritesStore.favoritesItems.slice(
...
const pageTotal = ref(favoritesStore.favoritesItems.length);
...
const getCatalog = async () => {
  ...
  favoritesStore.favoritesItems = await favoritesStore.getFavorites(
    authStore.userName
  );
};
definePageMeta({
...
const deliteItem = async (item) => {
  ...
  favoritesStore.favoritesItems = favoritesStore.favoritesItems.filter(
    (i) => i.favoritesItemId !== item.favoritesItemId
  );
};
</script>
